...
def test_sync_datasets_preserve_metadata_sync_columns(mocker: MockerFixture) -> None:
    """
    Test ``sync_datasets`` with preserve_metadata & sync_columns set to True.
    """
    client = mocker.MagicMock()
    get_or_create_dataset_mock = mocker.patch(
        "preset_cli.cli.superset.sync.dbt.datasets.get_or_create_dataset",
...
def test_sync_datasets_merge_metadata_sync_columns(mocker: MockerFixture) -> None:
    """
    Test ``sync_datasets`` with merge_metadata & sync_columns set to True.
    """
    client = mocker.MagicMock()
    get_or_create_dataset_mock = mocker.patch(
        "preset_cli.cli.superset.sync.dbt.datasets.get_or_create_dataset",
