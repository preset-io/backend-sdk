...
@click.pass_context
def accept_invites(ctx: click.core.Context, teams: List[str], path: str) -> None:
    """
    Auto-accept invites on behalf of the users.
    """
    client = PresetClient(ctx.obj["MANAGER_URL"], ctx.obj["AUTH"])
    ...
    with open(path, encoding="utf-8") as input_:
        config = yaml.load(input_, Loader=yaml.SafeLoader)
        emails = [user["email"] for user in config]
        client.accept_invites(teams, emails)


preset_cli.add_command(auth)
preset_cli.add_command(invite_users)
preset_cli.add_command(accept_invites)
